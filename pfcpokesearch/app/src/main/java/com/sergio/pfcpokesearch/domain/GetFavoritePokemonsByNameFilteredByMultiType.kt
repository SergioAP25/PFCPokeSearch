...
// filtrados por nombre y ordenados por multi tipo
class GetFavoritePokemonsByNameFilteredByMultiType @Inject constructor(
    private val repository: PokemonRepository
) {
    suspend operator fun invoke(name: String, type1: String, type2: String): List<FilteredPokemon> {
        val pokemons = repository.getFavoritePokemonsFromDatabaseByNameFilteredByMultiType(name, type1, type2)
        return pokemons
    }
}